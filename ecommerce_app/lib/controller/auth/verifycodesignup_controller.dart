...
import 'package:ecommerce_app/data/datasource/remote/auth/resetverifycode_data.dart';
...
//  late String verifycode;
  ...
  resend() async{
    statusRequest = StatusRequest.loading;
    update();
    var response = await resetverifycodeData.postData(email!);
    if (response == null) {
      statusRequest = StatusRequest.failed;
    }
    statusRequest = HandleData(response);
    if (StatusRequest.success == statusRequest) {
      if (response['status'] == 'success') {
             Get.defaultDialog(
            title: "Information", middleText: "We send another code please check your email");
      } else {
        Get.defaultDialog(
            title: "Warning", middleText: "The code is not correct please check your email !");
        statusRequest = StatusRequest.failed;
